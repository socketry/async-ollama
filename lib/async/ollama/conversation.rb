...
module Async
	module Ollama
		class Conversation
			...
			TRUNCATION_MESSAGE = "Messages before this point have been truncated and summarised by you. Continue the conversation naturally as if the previous messages were still present."
			
			def summarize!(retain = 0, role: "user")
				...
				if retain < @messages.size
					...
					@messages.unshift({
						role: "system",
						content: "**There have been #{truncated} previous messages**. #{TRUNCATION_MESSAGE}",
					})
				end
			end
		end
